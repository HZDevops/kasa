...
function AccomodationForm() {
  ...
  const accomodationSearched = AccomodationList.find(
  ...
  for (let index = 0; index < 5; index++) {
    ...
    if (fullStarRate === true) {
      accomodationRate.push(
        <img
          key={index}
          src={fullStar}
          alt={`${accomodationSearched?.rating}/5`}
        />
      );
    } else {
      accomodationRate.push(
        <img key={index} src={star} alt={`${accomodationSearched?.rating}/5`} />
      );
    }
  }

  //Get equipments list of the accomodation
  const equipments = accomodationSearched?.equipments.map(
    (equipment, index) => {
  ...
  return accomodationSearched ? (
    <div className="kasa-accomodation">
      ...
      <div className="accomodation-details">
        <DropDown
          title={"Description"}
          description={accomodationSearched?.description}
        />
